...
public:
    RequestHandler(const transport::Catalogue& catalogue, const renderer::MapRenderer& renderer, const transport::TransRouter& router)
    ...
    std::optional<transport::BusStat> GetBusInfo(const std::string_view bus_number) const;
...
private:
    ...
    const transport::TransRouter& router_;
};
