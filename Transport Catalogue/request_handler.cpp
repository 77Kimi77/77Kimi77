...
std::optional<transport::BusStat> RequestHandler::GetBusInfo(const std::string_view bus_number) const {
    std::optional<transport::BusStat> bus_stat = catalogue_.GetBusStat(bus_number);
    return bus_stat;
}

const std::set<std::string> RequestHandler::GetBusesByStop(std::string_view stop_name) const {
    return catalogue_.FindStop(stop_name)->buses_by_stop_;
}
...
svg::Document RequestHandler::RenderMap() const {
    return renderer_.RenderSVG(catalogue_.GetSortedAllBuses());
}
